...
begin
  ...
  if (iResult <> SizeOf(paquete)) then
    mmClientes.Lines.Add('Error de comunicación con '+Socket.RemoteAddress)
  else
    begin
...
procedure TfrmServidor.ServerSocket1ClientConnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
  mmClientes.Lines.Add('Conexión desde '+ Socket.RemoteAddress);
end;

procedure TfrmServidor.ServerSocket1ClientDisconnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
  mmClientes.Lines.Add('Desconexión de '+Socket.RemoteAddress);
end;
